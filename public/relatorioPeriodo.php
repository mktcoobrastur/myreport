...
<?php
    //CONSULTA PESQUISA
    $consulta = mysqli_query($con, "SELECT * FROM vendasre WHERE indice > '$inicio' AND indice < '$fim'");
    while($l = mysqli_fetch_array($consulta)) {
?>
    <tr>
      <td>
        <?php echo $l['representante']; ?>
      </td>
      <td>
        <span class="badge" style="font-size: 13px;"><?php echo $l['qnt']; ?></span>
      </td>
      <td align="right"><button type="button" class="btn-toggle" data-element=".minhaDiv"><?php echo date("d/m/Y", strtotime($l['indice'])); ?></button></td>
    </tr>
